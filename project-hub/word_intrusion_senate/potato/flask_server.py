...
print("cur_working_dir:" + cur_working_dir)
...
def load_all_data(config):
    ...
    for inst_id in instance_id_to_data:
        text = get_displayed_text(
            instance_id_to_data[inst_id][config["item_properties"]["text_key"]]
        )
        instance_id_to_data[inst_id]["displayed_text"] = re.sub(r" ", "<br>", text)
        instance_id_to_data[inst_id]["original_text"] = instance_id_to_data[inst_id][config["item_properties"]["text_key"]]
            ...
            for user_id, user_state in user_to_annotation_state.items():
                for inst_id, data in user_state.get_all_annotations().items():
                    ...
                    output = {
                        "instance_id": inst_id,
                        "user": user_id,
                        "displayed_text": instance_id_to_data[inst_id]["original_text"],
                        "label_annotations": data["labels"],
                        "span_annotations": data["spans"],
                        "behavioral_data": bd_dict,
                    }
    ...
    # Convert to Pandas and then dump
        ...
        for user_id, user_state in user_to_annotation_state.items():
            for inst_id, annotations in user_state.get_all_annotations().items():
                ...
                df["displayed_text"].append(instance_id_to_data[inst_id]["original_text"])
