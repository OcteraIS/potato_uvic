...
def generate_slider_layout(annotation_scheme):
    ...
    schematic = (
        ...
        + '  <input style="position:auto;" type="range" min="{min_value}" max="{max_value}" value="{starting_value}" class="slider" name="{name}" id="{name}" required oninput="updateSliderValue(this)" style="text-align: center;width: 500px;margin:0 auto">'
        + '  <span style="flex:1;">{max_label}</span>'
        + '  <right>[<span id="sliderValue_{name}" class="slider"></span>]'
        + '</fieldset>\n</form></div><br/>\n'
        + '<script>'
